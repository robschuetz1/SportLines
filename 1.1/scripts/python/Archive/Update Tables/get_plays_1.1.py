...
categories = [[rush_play_types, 'Rush'], [pass_play_types, 'Pass'], [eop, 'End of Period'], [penalty, 'Penalty'], [turnover, 'Turnover'], [special_teams, 'Special Teams']]
...
def get_adj_yardage(play):
    if (play.get("text").lower().find('penalty') != -1 and play.get("type").get("text") != 'Penalty') or play.get("text").lower().find('fumble') != -1:
        ...
        play_info = [play.get("id"), play.get("type").get("text"), play.get("text")]
        flag = 0
        position = []
...
def get_plays(drive_data):
    for drive in drive_data:
        ...
        for play in plays:
            ...
            if timeout_flag == 0:
                plays_to_add.append([drive_id, team_id, play_id, play_category, str(quarter), clock, str(down), str(distance), str(scoring_play), str(start_yard_line), str(end_yard_line), str(adj_yardage), str(yardage), desc])
...
#            if len(play[13]) > max_desc_length:
#                max_desc_length = len(play[13])
            play_count = play_count + 1
            cursor.execute('insert into plays values("' + play[0] + '", "' + play[1] + '", "' + play[2] + '", "' + play[3] + '", "' + play[4] + '", "' + play[5] + '", "' + play[6] + '", "' + play[7] + '", "' + play[8] + '", "' + play[9] + '", "' + play[10] + '", "' + play[11] + '", "' + play[12] + '", "' + play[13] + '")')
